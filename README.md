...
By default, only staged changes are committed. Use `--all` explicitly to include tracked and untracked changes. The extension generates a Conventional Commit message with the active Pi model, opens it for editing, and always asks for confirmation before committing.
